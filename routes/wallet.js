...
const getWalletModel = (prisma, type) => {
  switch (type.toLowerCase()) {
    case 'dodi': return prisma.dodiWallet;
    case 'dudi': return prisma.dudiWallet;
    case 'codi': return prisma.codiWallet;
    default: return null;
  }
};
...
router.post('/:username/create-:walletType-wallet', validateWalletType, async (req, res) => { // <-- Updated route path
  ...
  try {
    ...
    res.status(201).json({ message: `Wallet ${walletType} created successfully for user '${username}'`, wallet: newWallet });
  } catch (error) {
...
router.get('/wallet/:walletType/:walletId', validateWalletType, async (req, res) => { // <-- Updated route path
  ...
  try {
    ...
    logger.info(`Successfully fetched ${walletType} wallet details for ID '${walletId}' (Owner: ${wallet.user.username})`, { walletId: wallet.id, balance: wallet.balance });
    res.json(wallet);
  } catch (error) {
...
router.post('/wallet/:walletType/deposit', validateWalletType, async (req, res) => { // <-- Updated route path
  const { walletType } = req.params;
  ...
  try {
    ...
    logger.info(`Initiating deposit of ${depositAmount} for ${walletType} wallet '${walletId}' (Current Balance: ${targetWallet.balance})`);
    ...
    logger.info(`Deposit successful for ${walletType} wallet '${updatedWallet.id}'. New Balance: ${updatedWallet.balance}`);
    res.json({ message: 'Deposit successful', newBalance: updatedWallet.balance, walletId: updatedWallet.id });
  } catch (error) {
    logger.error(`Error during deposit for ${walletType} wallet '${walletId}': ${error.message}`, { stack: error.stack, depositAmount });
    res.status(500).json({ error: 'Internal server error' });
  }
});
...
router.post('/transfer', async (req, res) => {
  ...
  if (!senderWalletType || !senderWalletId || !receiverWalletType || !receiverWalletId || !amount) {
    logger.warn('Transfer attempt with missing parameters', { senderWalletType, senderWalletId, receiverWalletType, receiverWalletId, amount });
  ...
  try {
    await prisma.$transaction(async (tx) => {
      ...
      if (senderWallet.balance < transferAmount) {
        logger.warn(`Transfer failed: Insufficient balance for ${senderWalletType} wallet '${senderWalletId}'. Attempted: ${transferAmount}, Current: ${senderWallet.balance}`);
      ...
      logger.info(`Initiating transfer of ${transferAmount} from ${senderWalletType} '${senderWalletId}' to ${receiverWalletType} '${receiverWalletId}'`);
      ...
      logger.info(`Transfer successful: ${transferAmount} from ${senderWalletType} '${senderWalletId}' to ${receiverWalletType} '${receiverWalletId}'`);
      res.json({ message: 'Transfer successful' });
    });
  } catch (error) {
    logger.error(`Error during transfer from ${senderWalletType} '${senderWalletId}' to ${receiverWalletType} '${receiverWalletId}': ${error.message}`, { stack: error.stack, transferAmount });
...
router.get('/wallet/:walletType/:walletId/transactions', validateWalletType, async (req, res) => { // <-- Updated route path
  const { walletType, walletId } = req.params;
  const prisma = req.prisma;
  const logger = req.logger;
  const walletModel = getWalletModel(prisma, walletType);
  ...
  try {
    ...
    logger.error(`Error fetching transactions for ${walletType} wallet '${walletId}': ${error.message}`, { stack: error.stack });
    res.status(500).json({ error: 'Internal server error' });
  }
});
